...
def generate_test_data() -> None:
    ...
    generate_transactions_dataset(
        ...
        transactions_count=optimization_config.get_execution_config().get('transactions_count', 18000),
...
def analyze_results(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for exp_name, exp_results in experiment_groups.items():
        ...
        # Calculate average times (transformation only, not I/O)
        silver_times = [r['silver_layer']['transformation_time'] for r in successful_runs]
        ...
        total_with_io = [r['total_time_with_io'] for r in successful_runs]
        
        avg_silver = sum(silver_times) / len(silver_times)
        avg_gold = sum(gold_times) / len(gold_times)
        avg_total = sum(total_times) / len(total_times)
        avg_total_io = sum(total_with_io) / len(total_with_io)
        ...
        summary.append({
            ...
            'avg_total_time_with_io': avg_total_io,
    ...
    # Print summary table
    logger.info(f"{'Experiment':<30} {'Avg Silver':<12} {'Avg Gold':<12} {'Avg Total':<12} {'Min':<10} {'Max':<10}")
    logger.info("-" * 95)
        ...
        if baseline_time:
            ...
            for s in summary:
                if 'baseline' not in s['experiment']:
                    improvement = ((baseline_time - s['avg_total_time']) / baseline_time) * 100
                    logger.info(f"{s['experiment']:<30} {improvement:>6.2f}% {'faster' if improvement > 0 else 'slower'}")
