...
def setup_mock_views(spark_session) -> None:
    ...
    spark_session.createDataFrame(users_data, ["user_id", "name", "email", "country", "address", "signup_date"]).createOrReplaceTempView("users")
    ...
    spark_session.createDataFrame(products_data, ["product_id", "name", "category", "description", "price"]).createOrReplaceTempView("products")
    ...
    spark_session.createDataFrame(transactions_data, ["transaction_id", "user_id", "product_id", "quantity", "transaction_date"]).createOrReplaceTempView("transactions")
...
def test_top_spenders(spark_session, setup_mock_views) -> None:
    ...
    assert results[0]["name"] == "John", f"Expected top spender to be John, got {results[0]['name']}"
    assert results[0]["total_spent"] == 335.0, f"Expected John's total to be 335.0, got {results[0]['total_spent']}"
    
    # Verify second place is Maria
    assert results[1]["name"] == "Maria", f"Expected second spender to be Maria, got {results[1]['name']}"
    assert results[1]["total_spent"] == 210.0, f"Expected Maria's total to be 210.0, got {results[1]['total_spent']}"
    
    # Verify third place is Sarah
    assert results[2]["name"] == "Sarah", f"Expected third spender to be Sarah, got {results[2]['name']}"
    assert results[2]["total_spent"] == 170.0, f"Expected Sarah's total to be 170.0, got {results[2]['total_spent']}"
...
def test_top_products_by_category(spark_session, setup_mock_views) -> None:
    ...
    for category, products in results_by_category.items():
        assert len(products) <= 5, f"Category {category} has more than 5 products: {len(products)}"
...
def test_performance_analysis_by_country(spark_session, setup_mock_views) -> None:
    ...
    # Verify we have results
    assert len(results) > 0, "No results returned from performance_analysis_by_country query"
...
def test_day_to_day_sales(spark_session, setup_mock_views) -> None:
    ...
    if second_day["previous_day_sales"] > 0:
        expected_pct = ((second_day["total_sales"] - second_day["previous_day_sales"]) / 
                       second_day["previous_day_sales"]) * 100
